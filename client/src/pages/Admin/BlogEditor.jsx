...
const BlogEditor = ({ formData, onFormChange, onSave, onCancel }) => {
  ...
  // Ensure formData.headings is always an array with at least 5 empty sections
  const safeHeadings = Array.isArray(formData.headings) 
    ? [...formData.headings]
    : [];

  // Ensure we have exactly 5 sections
  while (safeHeadings.length < 5) {
    safeHeadings.push({ title: '', content: '' });
  }

  const handleSubsectionChange = (index, field, value) => {
    const updatedHeadings = [...safeHeadings];
    
    // Ensure the index is valid
    if (index >= 0 && index < 5) {
      // Update the specific field
      updatedHeadings[index] = {
        ...(updatedHeadings[index] || { title: '', content: '' }),
        [field]: value
      };
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl p-8 w-full max-w-4xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6"> {/* Simplified to single column layout */}
          ...
          <div>
            ...
            {formData.featured_image_url && (
              <div className="mt-4 p-2 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-inner">
                ...
                <img src={formData.featured_image_url} alt="Featured Image Preview" className="w-full h-48 object-cover rounded-md shadow" />
              </div>
            )}
          </div>
          {/* Tags Input */}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              {safeHeadings.map((subsection, index) => (
